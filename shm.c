...
buffer_release(void *data, struct wl_buffer *wl_buffer)
{
    ...
    assert(buffer->public.wl_buf == wl_buffer);
    assert(buffer->busy);
    buffer->busy = false;
...
{
    assert(buf->public.data == NULL);
    assert(buf->public.pix == NULL);
    assert(buf->public.wl_buf == NULL);
    assert(buf->pool != NULL);
    ...
    for (size_t i = 0; i < buf->public.pix_instances; i++) {
        ...
        if (cairo_surface_status(cairo_surfs[i]) != CAIRO_STATUS_SUCCESS) {
            LOG_ERR(
                "failed to create cairo surface: %s",
...
{
    assert(count == 1 || !chain->scrollable);
    /*
    ...
    assert(chain->scrollable || (offset == 0 && memfd_size == total_size));
...
shm_get_buffer(struct buffer_chain *chain, int width, int height, bool with_alpha)
{
    ...
    if (cached != NULL) {
        ...
        assert(cached->public.pix_instances == chain->pix_instances);
